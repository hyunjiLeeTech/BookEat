...
class RestaurantReservation extends Component {
    ...
    dataService
      ...
      .then((res) => {
        toast.update(infoToast, {
          render: "Not attend reported, thanks!",
          type: toast.TYPE.SUCCESS,
          autoClose: 5000,
          className: "pulse animated",
        });
        $("#" + reservationId + "btnNotAttend")
          .attr("disabled", "true")
          .text("Reported Not Attend");
      })
      .catch((err) => {
        if (err.errcode) {
          toast.update(infoToast, {
  ...
  cancelReservation(reservationId) {
    ...
    dataService
      ...
      .then((res) => {
        toast.update(infoToast, {
          render: "Reservation cancelled",
          type: toast.TYPE.SUCCESS,
          autoClose: 5000,
          className: "pulse animated",
        });
        $("#" + reservationId + "btnCancel")
          .attr("disabled", "true")
          .text("Cancelled");
      })
      .catch((err) => {
  ...
  renderPresent() {
    var rows = this.state.upcoming.map((ro, index) => (
      <tr key={index} id={ro._id}>
        <td>{ro.customer.firstName + " " + ro.customer.lastName}</td>

        <td>{ro.table.rid}</td>

        <td>{ro.dateTime}</td>

        <td>{ro.numOfPeople}</td>
        ...
        <td>
          {ro.FoodOrder ? (
            <button
              ...
              value={ro.FoodOrder}
              onClick={() =>
                this.viewFoodOrder(this.state.upcoming[index].FoodOrder)
              }
            >
              View Order
            </button>
        ...
        <td>
          <button
            ...
            onClick={() =>
              this.confirmAttandance(this.state.upcoming[index]._id)
            }
          >
            Confirm Attandance
          </button>
        ...
        <td>
          <button
            ...
            id={ro._id + "btnCancel"}
            onClick={() =>
              this.cancelReservation(this.state.upcoming[index]._id)
            }
          >
            Cancel Reservation{" "}
          </button>
        ...
        <td>
          {r.FoodOrder ? (
            <button
              ...
              id={r._id + "btn"}
              onClick={() =>
                this.viewFoodOrder(this.state.past[index].FoodOrder)
              }
            >
              View Order
            </button>
          ...
          <div className="tab-content">
            ...
            >
              <table className="table table-striped">
                ...
                <tbody>{this.renderPresent()}</tbody>
              </table>
            </div>
