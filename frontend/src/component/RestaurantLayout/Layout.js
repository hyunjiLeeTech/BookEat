...
class Layout extends Component {
  ...
  componentWillUpdate() {
    ...
    for (var i in tables) {
      ...
      var cn =
        theTable.prefers +
        " r-table col-md-4 hovereffect animated bounce" +
        (theTable.isOpen ? " open" : " close");
      ...
      row.push(
        <div key={theTable._id} className={cn + " r-table-s"} id={i}>
          <img
            src={tableImg}
            className={theTable.isOpen ? " img-responsive" : ""}
            alt="Table"
          />
          {theTable.isOpen ? (
  ...
  componentWillMount() {
    var tables = [];
    if (!this.state.tables) this.state.tables = tables;
    else {
  ...
  genCarousel(htmls) {
    ...
    for (var i in htmls) {
      ...
      if (i !== 0 && i % 3 === 0) {
        rows.push(
          <div key={"r" + rows.length} className="row">
            {tempItems}
          </div>,
        );
        tempItems = [];
    ...
    for (var j in rows) {
      ...
      if (j !== 0 && j % 3 === 0) {
        slides.push(
          <Carousel.Item key={"slide" + slides.length}>
    ...
    if (tempRows.length !== 0) {
      slides.push(
        <Carousel.Item key={"slide" + slides.length}>
          <div style={{marginLeft: "15%", marginRight: "15%"}}>{tempRows}</div>
        </Carousel.Item>,
      );
      tempRows = [];
    }
