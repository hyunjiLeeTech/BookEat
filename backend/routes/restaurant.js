...
router.post("/updaterespictures", async (req, res) => {
  ...
  await Restaurant.findOne({restaurantOwnerId: resOwner._id})
    .then((restaurant) => {
      restaurant.pictures = restaurant.pictures.filter(
        (p) => p != deletedImage,
      );
      restaurant.save();
      res.json({
        errcode: 1,
        errmsg: "res profile is updated with deleted image",
    ...
    .catch((err) => {
      res.json({
        errcode: 2,
        errmsg: "res profile is not updated with deleted image",
      });
    });
});

router.post(
  "/getRestaurantOwnerAndManagerViaRestaurantId",
  async (req, res) => {
    console.log("IN");
    var ro = await (
      await Restaurant.findOne({_id: req.body.restaurantId}).populate(
        "restaurantOwnerId",
      )
    ).restaurantOwnerId
      .populate("account")
      .execPopulate();
    var rm = await Manager.find({restaurantId: req.body.restaurantId}).populate(
      "accountId",
    );
...
router.route("/tableinfo").post(async (req, res) => {
  ...
  if (tables.length > 0) {
    ...
    for (var t of tables) {
      ...
      if (t.isOpen) {
        if (
          !(await isTableAvaliableAtTimeAsync(
            t,
    ...
    reservation
      ...
      .then(async (reservation) => {
        ...
        if (cus.noShowCount === 10) {
          ...
          await acc.save();
          var htmlMessage =
            "<h1>You missed your reservation.</h1><p>The restaurant just reported you missed an reservation which is at " +
            moment(new Date(reservation.dateTime)).format("YYYY-MM-DD HH:mm") +
            "</p>" +
...
router.route("/reserve").post(async (req, res) => {
  ...
  if (moment(obj.dateTime).diff(new Date(), "milliseconds") >= 2147483647) {
    return res.json({
      errcode: 5,
      errmsg: "Cannot reserve in more that 24 days in advance",
    });
  }

  var table = await Table.findOne({_id: obj.tableId}).populate("restaurant");
  var eatingTime = table.restaurant.eatingTime
    ? table.restaurant.eatingTime
    : 2;
  if (!isRestaurantAvailableAtDateTime(table.restaurant, req.body.dateTime))
    return res.json({
      errcode: 6,
  ...
  if (
    ...
    res.json({
      errcode: 3,
      errmsg: "Table is already reserved, please choose another table",
    });
    return;
  }
    ...
    rev
      ...
      .then(async (revs) => {
        ...
        var popedRevs = await revs
          ...
          .execPopulate();
        var htmlMessage =
          "<h1>Thanks for booking at BookEat</h1>" +
          "<h3>Here is your booking information:</h3>" +
          "<p>Customer name: " +
...
router.route("/reservationsofpast14days").get(async (req, res) => {
  ...
  if (u.userTypeId === 2) {
    ...
    var reservations = await Reservation.find({
      ...
      dateTime: {$gte: moment(new Date()).add(-14, "d").toDate()},
    })
      .populate("customer")
      .populate("table");
...
router.route("/upcomingreservations").get(async (req, res) => {
  ...
  if (u.userTypeId === 2) {
    ...
    var reservations = await Reservation.find({status: 2, restaurant: rest})
      .populate("customer")
      .populate("table");
    //.where('table.restaurant', u.restaurantId);
    //console.log(reservations)
...
let editRestaurantProfile = async (obj) => {
  ...
  if (obj.cuisineStyleVal == "" || obj.categVal == "" || obj.priceName == "") {
    status = 2;
  }
  ...
  //open and close time
  await StoreTime.findOne({storeTimeVal: obj.monOpenTime}).then((storeTime) => {
    monOpenId = storeTime._id;
  });

  await StoreTime.findOne({storeTimeVal: obj.tueOpenTime}).then((storeTime) => {
    tueOpenId = storeTime._id;
  });

  await StoreTime.findOne({storeTimeVal: obj.wedOpenTime}).then((storeTime) => {
    wedOpenId = storeTime._id;
  });

  await StoreTime.findOne({storeTimeVal: obj.thuOpenTime}).then((storeTime) => {
    thuOpenId = storeTime._id;
  });

  await StoreTime.findOne({storeTimeVal: obj.friOpenTime}).then((storeTime) => {
    friOpenId = storeTime._id;
  });

  await StoreTime.findOne({storeTimeVal: obj.satOpenTime}).then((storeTime) => {
    satOpenId = storeTime._id;
  });

  await StoreTime.findOne({storeTimeVal: obj.sunOpenTime}).then((storeTime) => {
