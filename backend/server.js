  ...
  if (Number.parseInt(externalType) === 1) {
    Axios({
      ...
      .then(async (resp) => {
        ...
        if (external)
          return res.json({errcode: 2, errmsg: "This email is already used."});
        if (account)
          return res.json({errcode: 3, errmsg: "This email is already used."});
        var obj = {
...
app.get("/testAuth", passport.authenticate("jwt", {session: false}), function (
  req,
  res,
) {
...
app.post("/search", async (req, res) => {
  ...
  try {
    ...
    console.log(keyword);
    var availableTables = await getTablesWithRestaurantsUsingPersionAndDateTimeAsync(
